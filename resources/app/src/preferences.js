...
function initPreferences() {
	...
	// 魔改：浏览器来源已禁用（DPAPI/App-Bound 加密，yt-dlp#10927），历史残留值强制回退到 file
	let initialCookieSource = savedCookieSource;
	if (initialCookieSource === "browser") {
		initialCookieSource = "file";
		localStorage.setItem("cookieSource", "file");
	}

	if (cookieSourceSelect) {
		cookieSourceSelect.value = initialCookieSource;
		updateCookieSourceUI(initialCookieSource);
	...
	// 魔改：应用内登录 YouTube 一键导入 cookies（主进程登录窗导出 → 存为 Netscape cookie 块 → 原生同步到 cookies.txt）
	getId("loginYoutubeBtn")?.addEventListener("click", async () => {
		const btn = getId("loginYoutubeBtn");
		const statusEl = getId("loginStatusTxt");
		if (!btn || btn.disabled) return;
		btn.disabled = true;
		if (statusEl) statusEl.textContent = "正在打开 YouTube 登录窗口…";
		try {
			const result = await ipcRenderer.invoke("open-youtube-login");
			if (result && result.success && result.cookiesText) {
				localStorage.setItem("cookieSource", "file");
				if (cookieSourceSelect) cookieSourceSelect.value = "file";
				updateCookieSourceUI("file");
				// 替换旧的登录导入块（按生成标记识别），手动粘贴的块不动
				cookieBlocks = cookieBlocks.filter(
					(b) =>
						!(
							b &&
							b.content &&
							b.content.includes("Generated by YTDownloader YouTube Login")
						),
				);
				cookieBlocks.push({
					id: Date.now().toString(),
					content: result.cookiesText,
				});
				saveAndSyncCookieBlocks(true);
				renderCookieBlocks();
				if (statusEl)
					statusEl.textContent = `已导入 ${result.count || 0} 条 cookies`;
			} else {
				if (statusEl)
					statusEl.textContent =
						(result && result.error) || "登录未完成或未检测到登录态";
			}
		} catch (err) {
			if (statusEl)
				statusEl.textContent = "登录失败: " + ((err && err.message) || err);
		} finally {
			btn.disabled = false;
		}
	});
