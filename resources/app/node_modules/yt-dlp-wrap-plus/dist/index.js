...
var YTDlpWrap = /** @class */ (function () {
    ...
    YTDlpWrap.processMessageToFile = function (message, filePath, onProgress, timeoutMs) {
        if (timeoutMs === void 0) { timeoutMs = 15000; }
        return new Promise(function (resolve, reject) {
            var _a;
            var cleanedUp = false;
            var total = Number((_a = message.headers['content-length']) !== null && _a !== void 0 ? _a : 0);
            var downloaded = 0;
            ...
            var cleanup = function (error) {
                ...
                (_a = message.socket) === null || _a === void 0 ? void 0 : _a.removeAllListeners('timeout');
                ...
                if (error) {
                    ...
                    fs_1.default.unlink(filePath, function (unlinkErr) {
                        reject(error);
                    });
                }
                else if (message.statusCode !== 200) {
                    fs_1.default.unlink(filePath, function (unlinkErr) {
                        reject(new Error("HTTP Status ".concat(message.statusCode)));
                    });
                }
                else {
                    resolve(message);
            ...
            timeoutId = setTimeout(function () {
                cleanup(new Error("Download timed out after ".concat(timeoutMs, "ms")));
            }, timeoutMs);
            message.on('data', function (chunk) {
                if (timeoutId) {
                    clearTimeout(timeoutId);
                    timeoutId = setTimeout(function () {
                        cleanup(new Error("Download timed out after ".concat(timeoutMs, "ms of inactivity")));
                    }, timeoutMs);
                }
            ...
            message.on('error', function (err) { return cleanup(err); });
            fileStream.on('error', function (err) { return cleanup(err); });
    ...
    YTDlpWrap.downloadFile = function (fileURL, filePath, onProgress) {
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 2:
                        ...
                        return [3 /*break*/, 5];
                    case 3: return [4 /*yield*/, YTDlpWrap.processMessageToFile(message, filePath, onProgress)];
    ...
    YTDlpWrap.downloadFromGithub = function (filePath_1, version_1) {
        return __awaiter(this, arguments, void 0, function (filePath, version, platform, onProgress, build) {
            var isWin32, isMac, isLinux, fileName, repoMap, repo, fileURL;
            if (platform === void 0) { platform = os_1.default.platform(); }
            if (build === void 0) { build = 'stable'; }
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        if (!filePath)
                            filePath = './' + fileName;
                        fileURL = version
                            ? "https://github.com/".concat(repo, "/releases/download/").concat(version, "/").concat(fileName)
                            : "https://github.com/".concat(repo, "/releases/latest/download/").concat(fileName);
                        return [4 /*yield*/, YTDlpWrap.downloadFile(fileURL, filePath, onProgress)];
                    case 1:
                        _a.sent();
                        if (!isWin32) {
                            fs_1.default.chmodSync(filePath, '777');
                        }
                        return [2 /*return*/];
                }
            });
